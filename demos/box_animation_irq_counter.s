VECTOR_TABLE:
//This program uses IRQ to display on the VGA display each time an exception occurs. It is setup so that the VGA throws an exception when it is done drawing. 
b START
...
SETUP_IRQ:
    ...
    mvt r4, #IRQ_ADDRESS
    mvt r0, #0x0F00 //Turn on IRQ from all switches
    add r0, #0x0F
...
IRQ:
    ...
    mvt r1, #LED_ADDRESS
    st r0, [r1] //Mark the total number of exceptiosn that occured onto the LED display
